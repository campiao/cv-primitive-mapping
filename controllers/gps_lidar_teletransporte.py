...
from controllers.utils import cmd_vel, plot_line, warp_robot
...
def main() -> None:
    ...
    while robot.step(timestep) != -1:
        ...
        if key == ord('W'):
            ...
            scan_count, current_count = teletransporte(waypoints, robot, scan_count, gps, compass,
                                                       lidar, map, current_count)
        elif key == ord('P'):
            points = map.get_grid()
...
def teletransporte(waypoints, robot, scan_count, gps, compass, lidar, map, current_count):
